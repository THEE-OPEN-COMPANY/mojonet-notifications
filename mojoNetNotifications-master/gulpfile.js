...
function devJS () {
  return gulp.src('src/*.coffee')
    ...
    .pipe(cof())
    .pipe($.concat('mojonet-notifications.js'))
    .pipe(write())
}
...
function jsmin () {
  return gulp.src('src/*.coffee')
    ...
    .pipe(cof())
    // Uglifiy => min.js
    .pipe($.concat('mojonet-notifications.min.js'))
    .pipe(ugl())
    .pipe(write())
}
...
function allJS () {
  return gulp.src(['node_modules/jquery/dist/jquery.min.js', 'node_modules/jquery.easing/jquery.easing.min.js', 'src/*.coffee'])
    ...
    .pipe($.if('*.coffee', cof()))
    .pipe(ugl())
    .pipe($.concat('mojonet-notifications.all.min.js'))
    .pipe(write())
}
...
function devCSS () {
  return gulp.src('src/*.css')
    .pipe($.sourcemaps.init())
    .pipe($.concat('mojonet-notifications.css'))
    .pipe(write())
}
...
function cssmin () {
  return gulp.src('src/*.css')
    ...
    .pipe($.concat('mojonet-notifications.css'))
    .pipe($.cleanCss())
    .pipe($.concat('mojonet-notifications.min.css'))
    .pipe(write())
}
